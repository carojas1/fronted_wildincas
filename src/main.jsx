  ...
  return <>
    ...
    <div className="data-table reservation-table"><div className="table-row table-head"><span>RESERVA / HUESPED</span><span>HABITACION</span><span>ESTADIA</span><span>TOTAL</span><span>ESTADO</span><span>ACCIONES</span></div>
      {rows.map((item) => <div className="table-row" key={item.id}><span><b>{item.code}</b><small>{item.guest?.name}</small><small>{item.guest?.documentNumber || "Consumidor final"}</small></span><span><b>Hab. {item.roomId}</b><small>{item.roomType}</small><small>{money(item.nightlyRate)}/noche</small></span><span><b>{dateText(item.checkIn)} - {dateText(item.checkOut)}</b><small>{item.nights} noche(s), {item.adults} adulto(s)</small><small>{item.charges?.length || 0} consumo(s)</small></span><span><b>{money(item.total)}</b><small>{money(paymentTotal(data.payments, item.id))} pagado</small><small>{money(Math.max(0, item.total - paymentTotal(data.payments, item.id)))} pendiente</small></span><span><Status status={item.status} /></span><span className="row-actions">
        ...
        {["confirmed", "checked_in"].includes(item.status) && <IconButton title="Editar" onClick={() => setModal({ type: "edit", item })}><Pencil size={16} /></IconButton>}
        {item.status === "confirmed" && <IconButton title="Check-in" onClick={() => checkIn(item)}><LogIn size={16} /></IconButton>}
        {item.status === "checked_in" && <IconButton title="Agregar consumo" onClick={() => setModal({ type: "charge", item })}><Plus size={16} /></IconButton>}
...
function Guests({ data, reload, notify }) {
  ...
  return <><div className="toolbar"><div className="search"><Search size={17} /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Nombre, cedula, correo o telefono" /></div><div className="counter">{rows.length} huespedes</div></div>
    <div className="guest-grid">{rows.map((guest) => { const stays = data.reservations.filter((item) => item.guestId === guest.id); return <article className="guest-card" key={guest.id}><header><Avatar name={guest.name} /><span><b>{guest.name}</b><small>{guest.country || "Nacionalidad no registrada"}</small></span><IconButton title="Editar" onClick={() => setEditing(guest)}><Pencil size={15} /></IconButton></header><dl><dt>Documento</dt><dd>{guest.documentType} {guest.documentNumber || "-"}</dd><dt>Correo</dt><dd>{guest.email || "-"}</dd><dt>Telefono</dt><dd>{guest.phone || "-"}</dd><dt>Direccion</dt><dd>{guest.address || "-"}</dd></dl><footer><span>{stays.length} estadia(s)</span><Status status={stays.some((item) => item.status === "checked_in") ? "active" : "inactive"} /></footer></article>; })}</div>
...
function Rooms({ data, reload, notify }) {
  ...
  async function save(values) {
    const method = values._new ? "POST" : "PATCH";
    const path = values._new ? "/rooms" : `/rooms/${values.id}`;
    const { _new, ...payload } = values;
    await request(path, { method, body: JSON.stringify(payload) }); notify(values._new ? "Habitacion creada" : "Habitacion actualizada"); setEditing(null); setCreating(false); reload();
  }
  return <><div className="toolbar"><StatusFilters values={["all", "available", "occupied", "cleaning", "maintenance", "out_of_service"]} active={filter} onChange={setFilter} /><button className="primary" onClick={() => setCreating(true)}><Plus size={17} /> Nueva habitacion</button></div>
    ...
    {creating && <RoomModal room={{ _new: true, id: "", floor: 1, type: "Habitacion Privada", capacity: 2, rate: 35, status: "available", notes: "" }} onClose={() => setCreating(false)} onSubmit={save} />}
...
function ReservationModal({ reservation, rooms, reservations, onClose, onSubmit }) {
  ...
  const [values, setValues] = useState(reservation ? { ...reservation, ...reservation.guest, action: reservation.status === "checked_in" ? "check_in" : "reserve" } : { name: "", documentType: "Cedula", documentNumber: "", email: "", phone: "", address: "", country: "", checkIn: today, checkOut: "", exitTime: "11:00", adults: 1, children: 0, roomId: "", nightlyRate: 0, source: "Recepcion", notes: "", action: "reserve" });
  const eligible = rooms.filter((room) => !["maintenance", "out_of_service"].includes(room.status) && !reservations.some((item) => item.id !== reservation?.id && item.roomId === room.id && ["confirmed", "checked_in"].includes(item.status) && values.checkIn < item.checkOut && values.checkOut > item.checkIn));
...
function ChargeModal({ reservation, onClose, onSubmit }) { const [values, setValues] = useState({ category: "Minibar", description: "", quantity: 1, unitPrice: 0, notes: "" }); return <Modal title={`Nuevo consumo - ${reservation.code}`} onClose={onClose}><div className="form-grid"><Field label="Categoria" type="select" options={["Restaurante", "Minibar", "Lavanderia", "Transporte", "Tour", "Servicio adicional"]} value={values.category} onChange={(category) => setValues({ ...values, category })} /><Field label="Descripcion" value={values.description} onChange={(description) => setValues({ ...values, description })} /><Field label="Cantidad" type="number" min="1" value={values.quantity} onChange={(quantity) => setValues({ ...values, quantity })} /><Field label="Precio unitario" type="number" value={values.unitPrice} onChange={(unitPrice) => setValues({ ...values, unitPrice })} /></div><Field label="Observaciones" type="textarea" value={values.notes} onChange={(notes) => setValues({ ...values, notes })} /><div className="calculation"><span><small>Total del consumo</small><b>{money(values.quantity * values.unitPrice)}</b></span></div><button className="primary full" disabled={!values.description || values.unitPrice <= 0} onClick={() => onSubmit(values)}><Plus size={16} /> Agregar a la estadia</button></Modal>; }
